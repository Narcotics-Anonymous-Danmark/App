...
function collectBlockers(platform, opts) {
  ...
  if (platform === 'ios') {
    ...
    if (!process.env.IOS_PROVISIONING_PROFILE_BASE64) {
      blockers.push(
        'IOS_PROVISIONING_PROFILE_BASE64 is required (App Store .mobileprovision, base64). ' +
        'Automatic signing cannot archive this project: cordova-ios pins ' +
        'CODE_SIGN_IDENTITY = "iPhone Distribution" in build-release.xcconfig, which Xcode ' +
        'refuses to combine with automatic signing. See README.md for how to produce it.'
      );
    }
    for (const name of ['APP_STORE_CONNECT_KEY_ID', 'APP_STORE_CONNECT_ISSUER_ID', 'APP_STORE_CONNECT_PRIVATE_KEY']) {
      if (!process.env[name]) blockers.push('missing signing secret: ' + name);
    }
    if (process.env.APP_STORE_CONNECT_PRIVATE_KEY) {
      try {
        normalizeApiPrivateKey(process.env.APP_STORE_CONNECT_PRIVATE_KEY);
      } catch (err) {
    ...
    if (process.platform === 'darwin' && !commandAvailable('xcodebuild')) {
      ...
      const sdk = capture('xcrun --sdk iphoneos --show-sdk-version', { soft: true, quiet: true });
      const major = sdk ? parseInt(String(sdk).trim().split('.')[0], 10) : NaN;
      if (Number.isFinite(major) && major < MIN_IOS_SDK_MAJOR) {
        blockers.push(
          `the selected Xcode builds against the iOS ${String(sdk).trim()} SDK, but App Store Connect ` +
          `requires iOS ${MIN_IOS_SDK_MAJOR} or later ("SDK version issue", HTTP 409) — ` +
          'select a newer Xcode with xcode-select, or set the XCODE_VERSION variable in CI'
        );
      }
    }
  }
...
function normalizeApiPrivateKey(raw, name) {
  const label = name || 'APP_STORE_CONNECT_PRIVATE_KEY';
  const bad = msg => { throw new Error(`${label} ${msg}`); };

  let text = String(raw == null ? '' : raw).trim();
  if (!text) bad('is empty');

  if (text.length > 1 && /^(['"])[\s\S]*\1$/.test(text)) text = text.slice(1, -1).trim();
  ...
  if (!text.includes('-----BEGIN')) {
    ...
    if (!/^[A-Za-z0-9+/]+={0,2}$/.test(compact)) {
      bad('is neither PEM text nor base64 — use the contents of AuthKey_<KEYID>.p8, or base64 of that file');
    }
    text = Buffer.from(compact, 'base64').toString('utf8').replace(/\r\n?/g, '\n').trim();
    if (!text.includes('-----BEGIN')) bad('is base64, but what it decodes to is not a PEM key');
  }

  text = text.split('\n').map(l => l.trim()).filter(Boolean).join('\n');

  const match = /-----BEGIN ([A-Z0-9 ]+)-----([\s\S]*?)-----END \1-----/.exec(text);
  if (!match) bad('has no complete -----BEGIN…-----END----- block');
  const kind = match[1];
  if (!/PRIVATE KEY$/.test(kind)) bad(`contains a "${kind}" block, not a private key`);

  const body = match[2].replace(/\s+/g, '');
  if (!body || !/^[A-Za-z0-9+/]+={0,2}$/.test(body)) bad('has a PEM header but its body is not valid base64');
  ...
  try {
    ...
    bad(`is a well-formed PEM but not a readable private key (${err.message})`);
  }
  if (parsed.asymmetricKeyType !== 'ec') {
    bad(`is an ${String(parsed.asymmetricKeyType).toUpperCase()} key — App Store Connect keys are EC (P-256); check you exported the right .p8`);
  }
