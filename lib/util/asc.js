...
function privateKeyFromEnv() {
  ...
  if (!raw) throw new Error('APP_STORE_CONNECT_PRIVATE_KEY is not set');
  // GitHub secrets keep real newlines, but accept the escaped form too.
  const key = raw.includes('BEGIN') ? raw.replace(/\\n/g, '\n') : Buffer.from(raw, 'base64').toString('utf8');
  if (!key.includes('BEGIN PRIVATE KEY')) {
    throw new Error('APP_STORE_CONNECT_PRIVATE_KEY does not look like a .p8 private key');
  }
  return key;
}
...
// Finds the uploaded build. `version` is CFBundleShortVersionString,
// `buildNumber` is CFBundleVersion.
...
// Apple takes minutes to process an upload; nothing can be done with the build
...
// "What to Test" for the build. Locale is a beta locale, independent of the
// store listing, so en-US always exists.
...
async function internalGroups(appId, name) {
  const res = await api('GET', `/v1/betaGroups?filter[app]=${appId}&limit=200`);
  const groups = (res.data || []).filter(g => g.attributes.isInternalGroup);
  if (!name) return groups;
  const match = groups.filter(g => g.attributes.name === name);
  if (match.length === 0) {
    throw new Error(`No internal TestFlight group named "${name}". Internal groups found: ${groups.map(g => g.attributes.name).join(', ') || '(none)'}`);
  }
  return match;
}
