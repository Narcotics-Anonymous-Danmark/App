...
function importCertificate(dryRun) {
  ...
  // A random per-run password: the keychain only has to survive this process.
  ...
  // Keep the login keychain in the search list so Apple's WWDR intermediates
  // stay resolvable, with ours first.
  ...
  if (!/\d+\)/.test(identities)) {
    release.fail('no code signing identity found in the temporary keychain', [
      'IOS_DIST_CERT_BASE64 must be a .p12 containing an Apple Distribution certificate *and* its private key',
      'export it from Keychain Access with "Export..." on the private key, not on the certificate alone'
    ]);
  }
  const identity = /"([^"]+)"/.exec(identities);
  console.log('Imported signing identity: ' + (identity ? identity[1] : 'unknown'));
}
...
function installProvisioningProfile(dryRun) {
  ...
  const target = path.join(release.homeDir(), 'Library', 'MobileDevice', 'Provisioning Profiles', `${uuid}.mobileprovision`);
  fs.mkdirSync(path.dirname(target), { recursive: true });
  fs.copyFileSync(raw, target);
  fs.rmSync(raw, { force: true });

  console.log(`Installed provisioning profile "${name}" (${uuid}${expires ? ', expires ' + expires[1] : ''})`);
  return { name, uuid, path: target };
}
...
function writeBuildConfig(profile, dryRun) {
  ...
  if (profile) {
    // PROVISIONING_PROFILE_SPECIFIER + manual signing style.
    ...
    if (!dryRun) {
      release.writeSecretFile(ios.authenticationKeyPath, process.env.APP_STORE_CONNECT_PRIVATE_KEY.replace(/\\n/g, '\n'));
    }
  }
  ...
  console.log(`Signing style: ${profile ? 'manual' : 'automatic (App Store Connect API key)'}`);
  return buildConfigPath;
}
...
function ensurePlatform(env, dryRun) {
  ...
  if (fs.existsSync(path.join(platformDir, 'Podfile')) || dryRun) {
    // `pod repo update` is minutes of churn; the CDN source only needs it when
    // a spec genuinely can't be resolved.
...
// Reads the version out of the built .ipa so a mislabelled artefact can never
// reach TestFlight.
...
function run(args) {
  ...
  try {
    ...
    if (!dryRun) {
      deleteKeychain();
      if (profile && profile.path) fs.rmSync(profile.path, { force: true });
