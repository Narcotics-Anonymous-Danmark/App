...
function importCertificate(dryRun) {
  ...
  // Certificates issued before ~2021 are named "iPhone Distribution: …", newer
  // ones "Apple Distribution: …". Use whichever is actually in the keychain
  // instead of guessing, and use the full name so a keychain holding several
  // identities can't sign with the wrong one.
...
function installProvisioningProfile(dryRun) {
  ...
  // Catch the two mismatches that otherwise surface as an opaque xcodebuild
  // "no profile matching" failure 20 minutes into the build.
