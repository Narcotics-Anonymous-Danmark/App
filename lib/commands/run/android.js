...
function runDeviceCmd(cmd, env, dryRun, retries) {
  if (dryRun) {
    console.log('> ' + cmd);
    return;
  }
  for (let attempt = 1; ; attempt++) {
    ...
    try {
      ...
      if (attempt > retries) {
        console.error('\nStill failing after ' + attempt + ' attempt(s). This is usually caused by an unstable USB/adb connection.');
