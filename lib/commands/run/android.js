...
function runSoftCmd(cmd, env, dryRun) {
  console.log('> ' + cmd);
  if (dryRun) return;
  try {
    execSync(cmd, { stdio: 'inherit', shell: true, env: env || process.env });
...
function emulatorAvdName(serial, env) {
  try {
    const out = execSync(`adb -s ${serial} emu avd name`, { encoding: 'utf8', shell: true, env });
...
function runningEmulators(env) {
  if (!commandAvailable('adb')) return [];
  let lines;
  try {
    const out = execSync('adb devices', { encoding: 'utf8', shell: true, env });
    lines = out.split('\n').slice(1).map(l => l.trim()).filter(Boolean);
  } catch (_) {
    return [];
  }
  ...
  for (const line of lines) {
    ...
    emulators.push({ serial, avd: emulatorAvdName(serial, env) });
  }
  return emulators;
}
...
function androidPackageName() {
  ...
  return (gradleConfig && gradleConfig.PACKAGE_NAMESPACE) || androidEnv.configAndroidPackageName();
}
...
function run(args) {
  ...
  const packageMismatch = androidEnv.androidPackageMismatch();
  if (packageMismatch) {
    console.log(`Android package id in platforms/android (${packageMismatch.actual}) does not match config.xml (${packageMismatch.expected})`);
  }

  if (toolchainIncomplete || packageMismatch) {
    console.log(toolchainIncomplete
      ? 'Android toolchain incomplete — running full bootstrap to reach a working state...'
      : 'Running bootstrap to regenerate the platform with the correct package id...');
  ...
  if (!useDevice) {
    const running = runningEmulators(env);
    if (running.length > 0) {
      const match = target
        ? running.find(e => e.avd === target || e.serial === target)
        : running[0];

      if (match) {
        reusedSerial = match.serial;
        target = match.serial;
        console.log(`Emulator ${match.serial}${match.avd ? ' (' + match.avd + ')' : ''} is already running — reusing it.`);
      } else {
        console.log(`AVD "${target}" is not running yet — it will be booted.`);
      }
    }
  }
  ...
  if (reusedSerial) {
    ...
    if (pkg) runSoftCmd(`adb -s ${reusedSerial} shell am force-stop ${pkg}`, env, dryRun);
  }
...
function usage() {
  ...
  console.log('--target <avd>  Override the AVD to use (default: a running emulator, else the last AVD listed)');
  console.log('An already-running emulator is reused: the app is force-stopped, reinstalled and relaunched on it instead of booting a second emulator.');
